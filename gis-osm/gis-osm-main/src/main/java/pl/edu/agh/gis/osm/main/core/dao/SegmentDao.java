...
public class SegmentDao extends BaseDao {
	...
	public Segment create(Segment segment) {
		...
		log.logSuccess(String.format("Created new Segment with id = %s", segment.getId()));
		return segment;
	}
	...
	public Segment getById(Integer id) {
		...
		Segment segment = segments.size() == 1 ? segments.iterator().next() : null;
		return segment;
	}
	...
	public List<Segment> getBySetId(Integer setId) {
		...
		List<Segment> segmentList = jdbcTemplate.query(GET_BY_SEGMENT_SET_ID_SQL, parameters, segmentCustomNodeExtractor);
		log.logSuccess(String.format("Returning %s Segments", segmentList.size()));
		return segmentList;
	}
}
